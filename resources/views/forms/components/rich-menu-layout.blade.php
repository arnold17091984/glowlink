<div {{ $attributes }} style="position: relative;">
    ...
    <x-filament::modal width="2xl" id="select-layout" :close-by-clicking-away="true">
        ...
        <div>
            <x-filament::tabs style="display:flex; max-width:900px;" x-data="{ activeTab: $wire.$entangle('selectedTab') }">
                <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center">
                    @for ($i = 1; $i <= 7; $i++)
                        <x-filament::tabs.item style="flex: 30%; max-width: 40%;"
                            alpine-active="activeTab === {{$i}}"
                            x-on:click="activeTab =  {{$i}}">
                            <img src="{{ asset('layout/richmenu/layout-' . $i . '.svg') }}" alt="Layout {{ $i }}"
